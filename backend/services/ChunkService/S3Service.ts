...
class S3Service implements ChunkInterface {
  ...
  getS3AuthThumbnail = async (
    ...
    if (thumbnail.personalFile) {
      ...
      return {
        s3Storage: s3Auth(s3Data.id, s3Data.key),
  ...
  streamVideo = async (
    ...
    if (fixedStart !== 0 && start !== 0) {
      ...
      currentIV = (await getPrevIVS3(
        fixedStart - 16,
        currentFile.metadata.s3ID!,
        isPersonal,
        user,
      )) as Buffer;
    }
    ...
    const params: any = {
      ...
      Range: `bytes=${fixedStart}-${fixedEnd}`,
    };
  ...
  getThumbnail = async (user: UserInterface, id: string) => {
    ...
    const { s3Storage, bucket } = await this.getS3AuthThumbnail(
      thumbnail,
      user,
    );
