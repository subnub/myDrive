...
const fields =
  'id, name, size, modifiedTime, hasThumbnail, parents, mimeType, thumbnailLink, webViewLink, shared';
...
class GoogleFileService {
  ...
  getList = async (user: UserInterface, query: googleQueryType) => {
    ...
    const convertedFiles = convertDriveListToMongoList(
      files.data.files,
      userID,
      nextPageToken,
    );
  ...
  getMongoGoogleList = async (user: UserInterface, query: googleQueryType) => {
    ...
    const sortedList = sortGoogleMongoList(
      [...convertedFiles, ...fileList],
      query,
    );
  ...
  getGoogleQuickList = async (user: UserInterface) => {
    console.log('getting google quick list', user);
    ...
    console.log('got google quick liust');
    return convertedFiles;
  };
  ...
  getGoogleMongoQuickList = async (user: UserInterface) => {
    ...
    const sortedGoogleMongoQuickList = sortGoogleMongoQuickFiles(
      convertedFiles,
      quickList,
    );
  ...
  getGoogleMongoSuggestedList = async (
    user: UserInterface,
    searchQuery: string,
  ) => {
    const { files, folders } = await googleDbFileUtils.getSuggestedList(
      searchQuery,
      user,
    );
    ...
    const convertedFolders = convertDriveFoldersToMongoFolders(
      folders.data.files,
      userID,
    );

    const {
      fileList: mongoFileList,
      folderList: mongoFolderList,
    } = await fileService.getSuggestedList(user._id, searchQuery);
  ...
  downloadFile = async (user: UserInterface, fileID: string, res: Response) => {
    const {
      fileMetadata,
      drive,
    } = await googleDbFileUtils.getDownloadFileMetadata(fileID, user);
  ...
  downloadDoc = async (user: UserInterface, fileID: string, res: Response) => {
    ...
    res.set(
      'Content-Disposition',
      'attachment; filename="' + fileMetadata.data.name! + '.pdf' + '"',
    );
    res.set('Content-Length', fileMetadata.data.size!);
