type FileDetailsIconType = React.SVGAttributes<SVGSVGElement>;

const FileDetailsIcon: React.FC<FileDetailsIconType> = (props) => {
  return (
    <svg
      width="38"
      height="50"
      viewBox="0 0 38 50"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g id="Group 9">
        <path
          id="Shape"
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M5.08748 0H37.5691V44.9219H32.4816V50H8.17078L0 41.8442V5.07812H5.08748V0ZM2.00417 42.1875L7.82689 47.9995V42.1875H2.00417ZM31.3076 48.8281H9.00092V41.0156H1.17403V6.25H31.3076V48.8281ZM32.4816 43.75H36.395V1.17188H6.26151V5.07812H32.4816V43.75Z"
          fill="#919EAB"
        />
        <path
          id="Path"
          d="M5.24863 11.3262H14.6409V12.4311H5.24863V11.3262Z"
          fill="#919EAB"
        />
        <path
          id="Path_2"
          d="M5.24863 14.3647H14.6409V15.4697H5.24863V14.3647Z"
          fill="#919EAB"
        />
        <path
          id="Path_3"
          d="M17.6796 11.3262H27.0718V12.4311H17.6796V11.3262Z"
          fill="#919EAB"
        />
        <path
          id="Path_4"
          d="M17.6796 14.3647H27.0718V15.4697H17.6796V14.3647Z"
          fill="#919EAB"
        />
        <path
          id="Shape_2"
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M27.9006 17.6797H4.69614V34.5305H27.9006V17.6797ZM26.7207 18.8553V30.9561L19.4447 23.7062L15.5118 27.625L13.152 25.2737L5.87602 32.5235V18.8553H26.7207ZM13.152 26.9364L6.7103 33.3549H26.7207V32.6185L19.4447 25.3689L16.346 28.4564L19.0752 31.1757L18.2409 32.007L13.152 26.9364Z"
          fill="#919EAB"
        />
        <path
          id="Path_5"
          d="M5.24863 36.4639H27.0718V37.5688H5.24863V36.4639Z"
          fill="#919EAB"
        />
        <path
          id="Path_6"
          d="M11.6022 39.5029H27.3481V40.6079H11.6022V39.5029Z"
          fill="#919EAB"
        />
        <path
          id="Path_7"
          d="M11.6022 42.5415H27.3481V43.6465H11.6022V42.5415Z"
          fill="#919EAB"
        />
      </g>
    </svg>
  );
};

export default FileDetailsIcon;
